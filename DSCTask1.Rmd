...
require(topicmodels) || install.packages("topicmodels")
require(lda) || install.packages("lda")
...
# txtdata <- txtdata.all
txtdata <- sample_n(txtdata.all,1000)
...
# NEXT STEPS
This is the initial exploratory data analysis. The processing time is important because the data -although is not very big- the combinations for igrams, or trigrams make the CPU heavy. Is is important to select the best model and efficient functions for the final model of the capstone.
The initial idea is to use the bigram and trigram inorder to predict the following word.
